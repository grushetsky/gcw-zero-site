<?php
$page = "specifications";
include "includes/header.php";
?>

<div id="content">
<h1>Specifications</h1>

<div class="table">
<div class="table-row"><div class="row0 text_bold">CPU:</div><div class="row0"><a href="files/JZ4770_DS.PDF">Ingenic JZ4770</a> 1 GHz MIPS processor</div></div>
<div class="table-row"><div class="row1 text_bold">GPU:</div><div class="row1">Vivante GC860, capable of OpenGL ES 2.0</div></div>
<div class="table-row"><div class="row0 text_bold">Display:</div><div class="row0">3.5-inch LCD with 320 × 240 pixels (4:3 aspect ratio is ideal for retro gaming)</div></div>
<div class="table-row"><div class="row1 text_bold">Operating system:</div><div class="row1">Linux 3.x (<a href="http://www.treewalker.org/opendingux/">OpenDingux</a>)</div></div>
<div class="table-row"><div class="row0 text_bold">Memory:</div><div class="row0">512 MB DDR2</div></div>
<div class="table-row"><div class="row1 text_bold">Internal storage:</div><div class="row1">16 GB, most of which is available for applications and data</div></div>
<div class="table-row"><div class="row0 text_bold">External storage:</div><div class="row0">microSDHC up to 32 GB or microSDXC of 64 GB (microSDXC cards must be reformatted before use)</div></div>
<div class="table-row"><div class="row1 text_bold">Connectivity:</div><div class="row1">Mini USB 2.0 OTG</div></div>
<div class="table-row"><div class="row1 text_bold"></div><div class="row1">Mini HDMI 1.3 out</div></div>
<div class="table-row"><div class="row1 text_bold"></div><div class="row1">3.5-mm (mini jack) A/V port for earphone and analog TV-out</div></div>
<div class="table-row"><div class="row0 text_bold">Audio:</div><div class="row0">Speakers, microphone</div></div>
<div class="table-row"><div class="row1 text_bold">Other:</div><div class="row1">Accelerometer (g-sensor) and vibration motors</div></div>
<div class="table-row"><div class="row0 text_bold">Wireless:</div><div class="row0">802.11b/g/n Wi-Fi 2.4 GHz, both access point and direct device-to-device connections are supported</div></div>
<div class="table-row"><div class="row1 text_bold">Dimensions:</div><div class="row1">143 × 70 × 18 mm</div></div>
<div class="table-row"><div class="row0 text_bold">Weight:</div><div class="row0">8 ounces (225 grams)</div></div>
<div class="table-row"><div class="row1 text_bold">Battery:</div><div class="row1">2200 mAh</div></div>
</div>

</div>
<div id="slideshow">
<img class="slideshow1" src="images/specifications/01.png" style="position: absolute;">
<img class="slideshow2" src="images/specifications/02.png" style="position: absolute;">
</div>

<?php
include "includes/footer.php";
?>
